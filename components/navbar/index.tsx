...
const CommandCustom = ({ size }: CommandCustomProps) => (
  <svg
    viewBox="0 0 20 20"
    height={size ?? 30}
    width={size ?? 30}
...
const Navbar = ({ title, variant }: NavbarProps) => {
  useEncoding('effectNavbar', 25)

  return (
    <nav
      className={clsx(
        variant === 'true' ? styles.nav_homepage : styles.nav_page
      )}
    >
      <div
        className={clsx(
      ...
      >
        ...
        <Command
          variantStyle="true"
          variantKey="true"
          variant={<CommandCustom size={32} />}
        />

        {title && (
          <div id="effectNavbar" className={styles.content}>
            {title}
          </div>
        )}
      </div>
